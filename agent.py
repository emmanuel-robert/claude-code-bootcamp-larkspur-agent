...
def run_agent(pnr: str, last_name: str, message: str) -> str:            # ✏️ Build 1, step 1.2
    ...
    while response.stop_reason == "tool_use" and turns < MAX_TOOL_CALLS:
        messages.append({"role": "assistant", "content": response.content})  # Fault 1: must echo full content list
        messages.append({"role": "user", "content": tool_results(response)})
        # Fault 2: removed stale `answer = text_of(response)` here
        response = client.messages.create(
            model=MODEL, max_tokens=4096, system=runtime_preamble() + SYSTEM_PROMPT + TONE_ADDENDUM,
            thinking={"type": "adaptive"}, tools=tools, messages=messages,
        )
    ...
    return text_of(response)  # Fault 3: return from final response, not stale var
